...
class GraphDataPreparationModule:
    ...
    def __init__(self, uri: str, user: str, password: str, database: str):
       ...
       """
        ...
        self.cooking_steps: List[GraphNode] = [] # 烹饪步骤
        ...
        try:
            ...
            with self.driver.session() as session:
                ...
                if record and record["test"] == 1:
                    logger.info("成功测试数据库连接")
        except Exception as e:
            logger.error(f"连接到图数据库失败: {e}")
            raise
    ...
    def load_graph_data(self):
        ...
        with self.driver.session() as session:
            ...
            logger.info(f"已加载 {len(self.recipes)} 条菜谱记录")
            ...
            logger.info(f"已加载 {len(self.ingredients)} 条食材记录")
            ...
            logger.info(f"已加载 {len(self.cooking_steps)} 条烹饪步骤记录")
